    ...
    <main>
      ...
      <div className="workspace">
        <section className="action-card" aria-busy={busy}>
          ...
          {task?.status === 'ReadyForPlanning' && <div className="ready-view"><div className="success-seal">✓</div><p className="eyebrow">REQUIREMENT APPROVED</p><h2>Ready for evidence-backed planning</h2><p>Forge will inspect the repository read-only, select bounded evidence, and create a deterministic Fake-mode plan.</p><div className="coming-next"><span>04</span><div><strong>Read-only planning</strong><p>No files, Git state, packages, builds, or tests will be changed.</p></div><button className="primary-button compact" onClick={() => void analyzeAndPlan()} disabled={busy}>Analyze repository and create plan</button></div><button className="text-button" onClick={startAnother}>Start another task</button></div>}
          {(task?.status === 'Planning' || planningProgress !== null && busy) && <div className="planning-progress"><div className="action-title"><span className="title-icon">04</span><div><h2>Creating an evidence-backed plan</h2><p>The target remains read-only throughout analysis.</p></div></div>{['Inspecting repository', 'Selecting evidence', 'Creating deterministic plan', 'Awaiting approval'].map((label, index) => <div className={`planning-progress-step ${index < (planningProgress ?? 0) ? 'complete' : index === (planningProgress ?? 0) ? 'active' : ''}`} key={label}><span>{index < (planningProgress ?? 0) ? '✓' : index + 1}</span><p>{label}</p></div>)}{!busy && <button className="secondary-button" onClick={() => void analyzeAndPlan()}>Re-analyze repository</button>}</div>}
          {task?.status === 'AwaitingPlanApproval' && task.implementationPlan && <div className="plan-view"><div className="action-title"><span className="title-icon">✓</span><div><h2>Review the deterministic plan</h2><p>Every proposed file is linked to selected repository evidence.</p></div></div><span className="fake-label">DETERMINISTIC FAKE-MODE OUTPUT · NO CODE CHANGED</span><section className="plan-section"><small>OBJECTIVE</small><p>{task.implementationPlan.objective}</p></section><section className="plan-section"><small>REPOSITORY UNDERSTANDING</small><p>{task.implementationPlan.repositoryUnderstanding}</p></section><section className="plan-section"><small>AFFECTED FILES</small>{task.implementationPlan.affectedFiles.map(file => <article className="planned-file" key={`${file.action}-${file.path}`}><div><code>{file.path}</code><span>{file.action}</span></div><p>{file.purpose}</p><small>Evidence: {file.evidenceIds.join(', ') || 'none'} · confidence {Math.round(file.confidence * 100)}%</small></article>)}</section><section className="plan-section"><small>ORDERED STEPS</small><ol>{task.implementationPlan.orderedSteps.map(step => <li key={step}>{step}</li>)}</ol></section><section className="plan-section"><small>PROPOSED VALIDATION · NOT RUN</small>{task.implementationPlan.proposedValidationCommands.map(command => <code className="validation-command" key={command}>{command}</code>)}</section><div className="plan-columns"><section className="plan-section"><small>RISKS</small><ul>{task.implementationPlan.risks.map(risk => <li key={risk}>{risk}</li>)}</ul></section><section className="plan-section"><small>ASSUMPTIONS</small><ul>{task.implementationPlan.assumptions.map(assumption => <li key={assumption}>{assumption}</li>)}</ul></section></div><div className="approval-row"><p><strong>Explicit plan approval required</strong><br />Approval records the gate; implementation remains unavailable.</p><div className="approval-actions"><button className="secondary-button" onClick={() => void analyzeAndPlan()} disabled={busy}>Re-analyze repository</button><button className="primary-button compact" onClick={approvePlan} disabled={busy}>Approve plan <span>→</span></button></div></div></div>}
          {task?.status === 'Implementing' && <div className="ready-view"><div className="success-seal">✓</div><p className="eyebrow">PLAN APPROVED</p><h2>Plan approved. Implementation is not available in this slice.</h2><p>The approved evidence and plan remain persisted for the next milestone. Forge did not modify the target repository.</p><button className="text-button" onClick={startAnother}>Start another task</button></div>}
