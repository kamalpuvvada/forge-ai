...
`GET /api/system/capabilities` reports clarification, planning, implementation, verification-planning, failure-analysis, correction, and delivery readiness independently, including explicit Fake/OpenAI availability. Dedicated commit, push, and pull-request fields become configured only when both Git and GitHub CLI are available; this is separate from per-task delivery eligibility and does not depend on Fake/OpenAI mode. Automated validation, auto-merge, and silent fallback remain false. No capability exposes a key, token, prompt, context identity, absolute workspace, or secret-derived data.
...
Plan and result text is untrusted. Local validators enforce character and UTF-8 budgets, approved command references, path containment, secret rejection, completion gates, and append-only history. The API exposes bound generation/start/update/complete commands and read-only verification-plan PDF export. `ReadyForDelivery` records an explicit user-reported pass only; it does not claim automated validation or delivery completion.

Delivery is a separate bounded workflow after that pass. Proposal preparation performs only structured read-only Git and `gh auth status` checks for GitHub.com, one `origin`, `main`, the approved base and worktree result, the protected active checkout, and one matching effective push destination. Exact approval is persisted without external mutation. Execution then stages only approved paths, creates one unsigned hook-disabled commit, revalidates the effective push destination, performs one non-force push to a Forge-owned branch, and opens one exact open pull request. The terminal state is `PullRequestCreated` with `NOT MERGED`; no merge action, arbitrary Git command, token input, automated validation, or generalized recovery UI exists.

## Submission-focused correction loop

Core adds immutable failure-analysis and correction-proposal records bound to the failed attempt, current failed result revisions, approved requirement and plan, initial implementation result, and original base commit. Strict Fake/OpenAI analysis can classify five routes; only an implementation defect can name an approved mutating path/action. SQLite uses additive normalized generation, analysis, proposal, approval, and correction-command tables with parent-format and immutable-ownership guards. Exact replay is semantic-command bound and an ambiguous or already-recorded correction command never causes a second provider dispatch.

Revision 2 is a complete operation set generated only after read-only verification of revision 1. It uses its own task-derived branch, ownership ref, lease, and linked worktree from the original base. All output is validated before reservation; non-correction operations must remain byte-identical to revision 1 and at least one approved correction operation must materially change. Revision 1 remains effective while revision 2 is pending or under review. Approval changes revision 1 to `HistoricallyApproved`, makes revision 2 effective, supersedes plan 1, and requires plan 2. Every current failed or blocked result revision must appear in a required regression case before plan 2 can be accepted.
...
Further correction revisions, candidate rejection/regeneration, requirement or approved-plan repair routes, environment retry, worktree cleanup or quota management, automated validation execution, merge, generalized delivery recovery, GitHub Enterprise, configurable remote/base selection, and production migrations are not part of this slice. Implementation generation still performs no stage, commit, push, or pull-request action; those mutations belong exclusively to the later explicitly approved bounded delivery stage.
