...
`ManualVerificationFailed` can generate a deterministic Fake or strict-schema OpenAI failure analysis. All five classifications are preserved, but only `ImplementationDefect` creates a locally derived exact-path/action correction proposal. Human approval authorizes one correction revision in a separate revision-2 worktree; revision 1 remains effective until the corrected diff is explicitly approved, then becomes historically approved. Plan 1 and attempt 1 remain immutable. Plan 2 must cover every failed or blocked result revision and a confirmed user-reported attempt-2 pass is required for `ReadyForDelivery`. No target command or delivery action is performed. OpenAI API billing remains separate from ChatGPT and Codex.
